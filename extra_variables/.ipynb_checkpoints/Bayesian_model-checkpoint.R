...
path2model<-here("extra_variables","m.archaeology_extra_variables.rds")
path2stan<-here("extra_variables","m.archaeology_extra_variables.stan")
...
img<-ifelse(is.na(data$image),2,1)
url<-ifelse(is.na(data$link),2,1)
...
m.archaeology_extra_variables <- stan(file=path2stan, data=d,iter=4000, chains=4,cores=4)


#This is the latest!
saveRDS(m.archaeology_extra_variables,path2model)
